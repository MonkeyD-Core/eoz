...
`contrib/init/eozid.service`, it has been changed to use
`/var/lib/eozid` as the data directory instead of
`~eozi/.eozi`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/eozid` will
exist has enough space (check using `df -h /var/lib/eozid`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/eozi/eozi/blob/master/doc/zmq.md#usage)
  for details.
...
`datadir` option in `/etc/eozi/eozi.conf` will have no effect.
...
- A new short [document](https://github.com/eozi/eozi/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/eozi/eozi/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/eozi/eozi/blob/master/doc/JSON-RPC-interface.md)
...
- The [output script
  descriptor](https://github.com/eozi/eozi/blob/master/doc/descriptors.md)
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/eozi/eozi/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/eozi/eozi/blob/master/doc/descriptors.md).
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/eozi/eozi/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/eozi/eozi/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart eozid with the `-deprecatedrpc=generate`
...
- #12490 Remove deprecated wallet rpc features from `eozi_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libeozi_server.a` (jnewbery)
...
- #13926 [Tools] eozi-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like eozid (MarcoFalke)
...
- #14521 qt, docs: Fix `eozi-qt -version` output formatting (hebasto)
...
- #14204 Move `interfaces/*` to `libeozi_server` (laanwj)
...
- #15154 Configure: eozi-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/eozi-wallet (Sjors)
...
- #13953 Fix deprecation in eozi-util-test.py (isghe)
...
- #12246 Bugfix: Only run eozi-tx tests when eozi-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_eozi` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_eozi` (MarcoFalke)
...
- #14186 eozi-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_eozi.conf` message on startup if conf file exists (leishman)
...
- #12255 Update eozi.service to conform to init.md (dongcarl)
...
- #14497 Add doc/eozi-conf.md (hebasto)
...
- #15354 Add missing `eozi-wallet` tool manpages (MarcoFalke)
...
- #15765 correct eoziconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support eozi folder on OSX (JimmyMow)
