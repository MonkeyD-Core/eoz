Eozi Core version 0.19.0.1 is now available from:

  <https://eozicore.org/bin/eozi-core-0.19.0.1/>
...
To receive security and update notifications, please subscribe to:
  ...
  <https://eozicore.org/en/list/announcements/join/>
...
installer (on Windows) or just copy over `/Applications/Eozi-Qt` (on Mac)
or `eozid`/`eozi-qt` (on Linux).

Upgrading directly from a version of Eozi Core that has reached its EOL is
possible, but might take some time if the datadir needs to be migrated.  Old
wallet versions of Eozi Core are generally supported.

Compatibility
==============

Eozi Core is supported and extensively tested on operating systems using
the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not recommended
to use Eozi Core on unsupported systems.
...
to open the Eozi Core .dmg. This is due to new signing requirements
imposed by Apple, which the Eozi Core project does not yet adhere too.
...
- [Reduce memory](https://github.com/eozi/eozi/blob/master/doc/reduce-memory.md)
  suggests configuration tweaks for running Eozi Core on systems with
...
- `-blockfilterindex` enables the creation of BIP158 block filters for
  ...
  currently use about 4 GiB of space.  Note: this version of Eozi
...
- `whitebind` and `whitelist` now accept a list of permissions to
  ...
  releases.  See the `eozid -help` output for these two options for
...
- In 0.18.0, a `./configure` flag was introduced to allow disabling BIP70 support in the GUI (support was enabled by default). In 0.19.0, this flag is now __disabled__ by default. If you want to compile Eozi Core with BIP70 support in the GUI, you can pass `--enable-bip70` to `./configure`. (#15584)
...
- `generate` is now removed after being deprecated in Eozi Core 0.18.
...
- BIP 61 reject messages were deprecated in v0.18. They are now disabled
  ...
  future version of Eozi Core. (#14054)

- To eliminate well-known denial-of-service vectors in Eozi Core,
  ...
  This prevents Eozi Core from sending the BIP111 NODE_BLOOM service
  ...
  of Eozi Core and nodes that have manually enabled BIP37 support,
...
- By default, Eozi Core will now make two additional outbound connections that are exclusively used for block-relay.  No transactions or addr messages will be processed on these connections. These connections are designed to add little additional memory or bandwidth resource requirements but should make some partitioning attacks more difficult to carry out. (#15759)

Miscellaneous CLI Changes
-------------------------

- The `testnet` field in `eozi-cli -getinfo` has been renamed to
...
- `getblockchaininfo` no longer returns a `bip9_softforks` object.
  ...
  an additional `type` field describes how Eozi Core determines
...
- When fetching a transaction announced by multiple peers, previous versions of
  Eozi Core would sequentially attempt to download the transaction from each
...
- If a Tor hidden service is being used, Eozi Core will be bound to
...
- Transactions with outputs paying v1 to v16 witness versions (future
  ...
  and services can accept any valid bech32 Eozi address without
...
- When creating a transaction with a fee above `-maxtxfee` (default 0.1
  EOZ), the RPC commands `walletcreatefundedpsbt` and
  `fundrawtransaction` will now fail instead of rounding down the fee.
  Be aware that the `feeRate` argument is specified in EOZ per 1,000
...
- A new wallet flag `avoid_reuse` has been added (default off). When
  ...
  partial spends" (added in Eozi Core v0.17.0), this can eliminate a
...
- #15649 Add ChaCha20Poly1305@Eozi AEAD (jonasschnelli)
...
- #15639 eozi-wallet tool: Drop libeozi_server.a dependency (ryanofsky)
...
- #16521 Use the default maxfeerate value as EOZ/kB (Remagpie)
...
- #16366 init: Use InitError for all errors in eozid/qt (MarcoFalke)
...
- #15239 scripts and tools: Move non-linux build source tarballs to "eozi-binaries/version" directory (hebasto)
...
- #16258 [MSVC]: Create the config.ini as part of eozid build (NicolasDorier)
...
- #16441 remove qt libjpeg check from eozi_qt.m4 (fanquake)
...
- #16750 msbuild: adds bench_eozi to auto generated project files (sipsorcery)
...
- #15773 Add EoziTestFramework::sync_* methods (MarcoFalke)
...
- #15352 Reduce noise level in test_eozi output (practicalswift)
...
- #15896 feature_filelock, interface_eozi_cli: Use PACKAGE_NAME in messages rather than hardcoding Eozi Core (luke-jr)
...
- #16277 Suppress output in test_eozi for expected errors (gertjaap)
...
- #16929 follow-up to rpc: default maxfeerate value as EOZ/kB (jonatack)
...
- #16489 log: harmonize eozid logging (jonatack)
...
- #17449 fix uninitialized variable nMinerConfirmationWindow (eoziVBR)

### Documentation
- #15514 Update Transifex links (fanquake)
- #15513 add "sections" info to example eozi.conf (fanquake)
...
- #15683 Comment for seemingly duplicate LIBEOZI_SERVER (Bushstar)
...
- #16080 build/doc: update eozi_config.h packages, release process (jonatack)
...
- #16448 add note on precedence of options in eozi.conf (fanquake)
...
- #16621 add default eozi.conf locations (GChuf)
...
- #16863 Add a missing closing parenthesis in the eozi-wallet's help (darosior)
- #16757 CChainState return values (MarcoFalke)
- #16847 add comments clarifying how local services are advertised (jamesob)
- #16812 Fix whitespace errs in .md files, eozi.conf, and Info.plist.in (ch4ot1c)
...
- #16912 Remove Doxygen intro from src/eozid.cpp (ch4ot1c)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/eozi/eozi/).
