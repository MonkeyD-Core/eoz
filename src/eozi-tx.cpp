...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " eozi-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  eozi-tx [options] <hex-tx> [commands]  Update hex-encoded eozi transaction\n" +
            "or:     eozi-tx [options] -create [commands]   Create hex-encoded eozi transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded eozi transaction
