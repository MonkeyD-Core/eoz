...
const char * const EOZI_CONF_FILENAME = "eozi.conf";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "eozid -foo=bar
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //eozi-tx using stdin
        std::string val;
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Eozi
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Eozi
    // Mac: ~/Library/Application Support/Eozi
    // Unix: ~/.eozi
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Eozi";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Eozi";
#else
    // Unix
    return pathRet / ".eozi";
#endif
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", EOZI_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure Eozi Core copyright is not removed by accident
    if (copyright_devs.find("Eozi Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Eozi Core developers";
    }
